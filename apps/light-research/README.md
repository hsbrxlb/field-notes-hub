...
The GitHub Actions `Clear expired research records` workflow calls the service's retention endpoint daily; a failed run must be investigated and rerun. The endpoint exposes no participant data and only removes sessions past their own stored policy deadline. The isolated PostgreSQL check is `SURVEY_RETENTION_DB=1 DATABASE_URL=<isolated database> npm test -- tests/retention-db.test.ts`. The ordinary liveness endpoint remains read-only.
